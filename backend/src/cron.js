    ...
    try {
      ...
      for (const med of medicines) {
        ...
        if (isOverdue) {
          ...
          const waLink = `https://wa.me/${contact.phone}?text=` + 
            encodeURIComponent(`Hi ${contact.name || 'there'}, ${med.user.name} hasn't taken ${med.name} yet today. Please check on them. 💙`);
          
          console.log(`[ALERT] Whatsapp Link Generated for missed dose check.`);
          // In a real app, this might insert a notification record to the DB or use Twilio to auto-send.
        }
      }
    } catch (err) {
